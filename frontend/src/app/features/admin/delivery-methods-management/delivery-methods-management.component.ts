    ...
    if (confirm(`Êtes-vous sûr de vouloir supprimer "${method.name}" ?`)) {
      this.ordersService.deleteDeliveryMethod(method.id).subscribe({
        ...
        error: (error) => {
          ...
          // Si la méthode est utilisée par des commandes, proposer de la désactiver
          if (errorMessage.includes('utilisée par') && errorMessage.includes('commande')) {
            if (confirm(`${errorMessage}\n\nVoulez-vous la désactiver à la place ?`)) {
              this.toggleActive(method);
            }
          } else {
